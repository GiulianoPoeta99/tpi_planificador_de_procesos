...
class ExternalPriority(PolicyStrategy):
	def __init__(self, scheduler: ProcessScheduler, logger: CustomLogger):
		...
		self.sort_ready_queue(1)

	def advance_time_unit(self):
		self.logger.log_ws()
		self.time_unit += 1
		
	def sort_ready_queue(self, donde):
		self.ready_queue.sort(key=lambda process: process.priority, reverse=True)
		self.logger.info(self.time_unit, f'{donde}', f"Ready Queue sorted by priority: {[p.id for p in self.ready_queue]}")
	...
	def update_ready_queue(self):
		...
		for process in self.scheduler.processes:
			if process.arrival_time == self.time_unit:
				...
				self.logger.info(self.time_unit, 'READY', f"Process '{process.name}' (pid: {process.id}) was added to Ready Queue")
